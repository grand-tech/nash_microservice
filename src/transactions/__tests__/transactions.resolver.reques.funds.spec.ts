    ...
    validateFundsRequest: () => {
        return {
            ...
            body: {
                ...
                stableCoin: 'cUSD',
                network: 'CELO',
                initiatorAddress: '0xFf1A74330dd9e899E5a857E251F8880554F6A260',
...
describe('TRANSACTION RESOLVER : REQUEST FUNDS SERVICE MUTATION : SUIT ', () => {
    ...
    beforeAll(async () => {
        const moduleRef = await Test.createTestingModule({
            ...
            .overrideProvider(RequestFundsService)
            .useValue(requestFundsServiceMock)
            .compile();
    ...
    it(`Check if requestFunds mutation exists.`, () => {
        const mutation = `mutation {
            requestFunds(
                targetPhoneNumber: "+254791235651",
                usdAmount: 0.00001,
        ...
        return request(app.getHttpServer())
            ...
            .expect(200)
            .expect((res) => {
                const rsp = res.body.data.requestFunds;
    ...
    it(`Check if fulfillFundsRequest mutation exists.`, () => {
        const mutation = `mutation {
            fulfillFundsRequest(requestID: 1) {
                status
                message
                body {
                    stableCoin
        ...
        return request(app.getHttpServer())
            ...
            .expect(200)
            .expect((res) => {
                const rsp = res.body.data.fulfillFundsRequest;
                expect(rsp.status).toBe(200);
                expect(rsp.message).toBe('Success');
